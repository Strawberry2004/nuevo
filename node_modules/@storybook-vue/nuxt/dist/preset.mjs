import path, { join, dirname as dirname$1 } from 'node:path';
import { fileURLToPath, pathToFileURL } from 'node:url';
import { createRequire } from 'node:module';
import { dirname, resolve as resolve$2, normalize } from 'pathe';
import { resolvePath } from 'mlly';
import { createFilter, isCSSRequest, normalizePath as normalizePath$1, transformWithEsbuild, formatPostcssSourceMap, mergeConfig, searchForWorkspaceRoot } from 'vite';
...
const runtimeDir = resolve$2(distDir$1, "runtime");
const pluginsDir = resolve$2(runtimeDir, "plugins");
const componentsDir = resolve$2(runtimeDir, "components");
const composablesDir = resolve$2(runtimeDir, "composables");
resolve$2(distDir$1, "..");
...
const packageDir = resolve$2(fileURLToPath(import.meta.url), "../..");
const distDir = resolve$2(fileURLToPath(import.meta.url), "../..", "dist");
...
function mergeViteConfig(storybookConfig, nuxtConfig, nuxt) {
  const extendedConfig = mergeConfig(nuxtConfig, storybookConfig);
...
function createResolver(base) {
  ...
  if (base.startsWith("file://")) {
    base = dirname$1(fileURLToPath(base));
  }
  return {
    resolve: (...path) => resolve$2(base, ...path),
...
async function getPackageDir(packageName) {
  try {
    ...
    return dirname$1(require.resolve(join(packageName, "package.json")));
  } catch (e) {
