...
const replace__default = /*#__PURE__*/_interopDefaultCompat(replace);
...
const _require = node_module.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href)));
...
const distDir$1 = pathe.dirname(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))));
const runtimeDir = pathe.resolve(distDir$1, "runtime");
const pluginsDir = pathe.resolve(runtimeDir, "plugins");
const componentsDir = pathe.resolve(runtimeDir, "components");
const composablesDir = pathe.resolve(runtimeDir, "composables");
pathe.resolve(distDir$1, "..");

function nuxtRuntimeConfigPlugin(runtimeConfig) {
...
const packageDir = pathe.resolve(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))), "../..");
const distDir = pathe.resolve(node_url.fileURLToPath((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href))), "../..", "dist");
...
async function extendComposables(nuxt) {
  ...
  addImportsSources({
    imports: ["useRouter"],
    from: path.join(composablesDir, "router")
  });
}
async function loadNuxtViteConfig(root) {
  ...
  nuxt.options.build.transpile.push(path.join(packageDir, "preview"));
...
function mergeViteConfig(storybookConfig, nuxtConfig, nuxt) {
  const extendedConfig = vite.mergeConfig(nuxtConfig, storybookConfig);
  ...
  return vite.mergeConfig(extendedConfig, {
...
function createResolver(base) {
  ...
  if (base.startsWith("file://")) {
    base = path.dirname(node_url.fileURLToPath(base));
  }
  return {
    resolve: (...path) => pathe.resolve(base, ...path),
    async resolveModule(id, options) {
      const paths = options?.paths ?? [base];
      paths.concat([base]);
      return await mlly.resolvePath(id, { url: paths }).catch(() => null);
    }
  };
}
const previewAnnotations = async (entry = []) => {
  const resolver = createResolver((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.src || new URL('preset.cjs', document.baseURI).href)));
  return [
    ...entry.map((entry2) => {
      if (typeof entry2 === "string" && entry2.includes("vue3")) {
        return {
          bare: pathe.normalize(entry2),
          absolute: ""
        };
      } else {
        return entry2;
...
const viteFinal = async (config, options) => {
  const getStorybookViteConfig = async (c, o) => {
    const presetURL = node_url.pathToFileURL(
      path.join(await getPackageDir("@storybook/vue3-vite"), "preset.js")
    );
...
async function getPackageDir(packageName) {
  try {
    ...
    return path.dirname(require$1.resolve(path.join(packageName, "package.json")));
  } catch (e) {
...
exports.core = core;
exports.getNuxtProxyConfig = getNuxtProxyConfig;
